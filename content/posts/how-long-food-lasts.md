...
excerpt: "How long milk, chicken, mince, eggs, cheese and leftovers last in the fridge, in one chart built from CSIRO and Australian food-safety guidance, sourced row by row."
tag: STORAGE
featured: true
...
There is no yoghurt row above, on purpose. CSIRO has never published a yoghurt figure, and the number repeated everywhere else on the internet is not CSIRO’s. Where we do not have a real source, we leave the gap visible rather than paper over it with someone else’s guess. The same goes for a dozen other pantry staples — flour, tinned goods, dry rice — that keep for so long the question is closer to “is it still good” than “when does it expire”, which is a different problem to the one this page solves.

## Fridge, pantry or freezer

Not everything on this list belongs in the fridge in the first place. Bananas, tomatoes, whole pumpkin and unpeeled garlic last *longer* out of it — putting them in the crisper is the mistake, not the fix. See the full breakdown, item by item, in [where to store fruit and veg](/resources/fruit-veg-storage-guide).

Freezing changes the question entirely: from a food-safety point of view, most frozen food keeps indefinitely, and the real limit is quality, not risk. That is its own post, not a row in this table — see [freezer times, explained](/resources/freezer-times-explained).

## Two dates that are not the same date

“Best before” and “use by” get used interchangeably in conversation and mean two different things on the label. Getting that distinction right saves more food than any individual shelf-life number does — see [best before vs use by](/resources/best-before-vs-use-by).
...
What a chart like this is for is not needing to guess in the first place — [keeping track of the fridge](/resources/how-to-keep-track-of-food-in-the-fridge) starts from the receipt, not from opening the door. Pantry is built to keep this same dataset behind every item a receipt adds, so the expiry date on the list is this table, not a memory of it. [How Pantry reads a receipt](/resources/how-pantry-reads-a-receipt) has the detail on how that works, and the [household food waste numbers](/resources/household-food-waste-by-the-numbers) explain why a few days one way or the other adds up to real money over a year.
